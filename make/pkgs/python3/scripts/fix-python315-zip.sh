#!/bin/bash
#
# Fix Python 3.15 zip importer compatibility
# This script fixes python315.zip to make it compatible with Python 3.15's zip importer
# by copying .pyc files from __pycache__/ to package level
#
# Usage: fix-python315-zip.sh <path_to_python315.zip>
#

set -e

if [ $# -ne 1 ]; then
    echo "Usage: $0 <path_to_python315.zip>"
    exit 1
fi
...
echo "Fixing Python 3.15 zip importer compatibility..."
...
while IFS= read -r pycache_dir; do
    ...
    for pyc_file in "$pycache_dir"/*.cpython-315.pyc; do
        if [ -f "$pyc_file" ]; then
            module_name=$(basename "$pyc_file" .cpython-315.pyc)
...
echo "Python 3.15 zip importer fix completed successfully!"
