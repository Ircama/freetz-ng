...
# Eventualmente:
rm .github/workflows/* # Cancella tutti i workflow (si può tenere make_tester.yml)
...
-----------------------
gh release delete none --yes
git tag -d none
git push origin :refs/tags/none
URL=$(gh release create none -t ".config" -n ".config" --prerelease .config | sed 's#/releases/tag/#/releases/download/#; s#$#/default.config#')
#gh workflow run make_tester.yml -f url="$URL"
gh workflow run make_tester.yml -f url="$URL" -f verbosity="1" -f download_toolchain="true"
gh workflow run make_package.yml -f make_target="patchelf" -f url="$URL" -f verbosity="2"
gh run list -w make_tester
gh run view 19010431318 --log
gh run watch 19010431318
